...
int main()
{
	...
	unsigned long long * verification_key = malloc(sizeof(unsigned long long) * N_SUB_CHAIN * OWF_N_BIT);
	...
	for (i = 0; i < N_SUB_CHAIN; i++)
	{
		...
		for (j = 0 ; j < OWF_N_BIT; j++)
		{
			verification_key[i*OWF_N_BIT + j] = node[j];
		}
	}
	...
	for (i = 0; i < N_SUB_CHAIN * OWF_N_BIT; i++)
	{
		for (j = 0; j < 8; j ++)
		{
			temp = (verification_key[i] >> (j*8)) & 0xff;
			shs256_process(&sh,temp);
		}
	}
	...
	printf("Replenishment: Multi_chain (%d sub-chains) takes %lf seconds to replenishment in the standard model\n", N_SUB_CHAIN, elapsed_secs);
